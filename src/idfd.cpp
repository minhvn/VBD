...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/idf.conf are parsed in qt/dash.cpp's main()
    ...
    if (mapArgs.count("-?") || mapArgs.count("-h") ||  mapArgs.count("-help") || mapArgs.count("-version"))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  idfd [options]                     " + _("Start Dash Core Daemon") + "\n";
    ...
    try
    {
        ...
        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in idfd anymore. Use the idf-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
#ifndef WIN32
